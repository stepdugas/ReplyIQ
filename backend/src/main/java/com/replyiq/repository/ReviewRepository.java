...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT r FROM Review r WHERE r.location.user.id = :userId ORDER BY r.postedAt DESC")
    List<Review> findAllByUserId(Long userId);

    @Query("SELECT r FROM Review r WHERE r.location.user.id = :userId AND r.replyStatus = :status ORDER BY r.postedAt DESC")
